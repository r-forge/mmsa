...
scoreSequence <- function(x, newdata, ...) UseMethod("scoreSequence")

scoreSequence.GenModel <- function(x, newdata, method = c("prod", "malik",
		"misstran",""), match_cluster="nn", plus_one = FALSE,
	initial_transition = FALSE, ...) { 
    
    # for Kullback method add 1
    if(x$model@measure=="Kullback") newdata <- newdata+1

	    method <- match.arg(method)
            
			if(method == "prod" || is.na(method) ||
				length(method)==0) { score(x$model,
				newdata=newdata, method="prod",
				match_cluster=match_cluster, plus_one=plus_one,
				initial_transition=initial_transition) }

	    if(method == "malik") { emm_test<-build(emm_test,newdata)
	    distance<-0
	    d<-dist(cluster_centers(emm_test),cluster_centers(x$model))
	    closestClusters<-apply(d, MARGIN=1, FUN=which.min) for(i in
		    1:length(closestClusters)) { closestDist <-
	    d[i,closestClusters[i]] #check if previous transition exists in
	    model
	    previousTransition<-transition(x$model,as.character(closestClusters[i]),as.character(closestClusters[i]-1))
	    if (previousTransition > 0) previousTransition=1 else if
	    (previousTransition ==0) previousTransition =
	    -log(1/nstates(emm_test));
						
						distance<-distance+(closestDist*previousTransition)
					    } #for each state in emm_test find
					    closest cluster #for(i in
					    1:nstates(emm_test)) #{
				#	d<-numeric(nstates(x$model)) for(j in
				    #	1:nstates(x$model)) {
				    #	d[j]<-dist(rbind(cluster_centers(emm_test)[i,],cluster_centers(x$model)[j,]),method="Manhattan")
				    #	#find min distance to get closest
				    #	cluster closest<-which.min(d)
				    #	closestDist <- min(d) #check if
				    #	previous transition exists in model
				#		previousTransition<-transition(x$model,as.character(closest),as.character(closest-1))
				    #		if (previousTransition > 0)
				    #		previousTransition=1 else if
	    ...
	    if(method == "misstran") { transitionTable <-
	    transition_table(x$model, newdata, method="prob", match_cluster,
		    plus_one, initial_transition) missingTransitions <-
	    sum(transitionTable[,3]==0) #missingTransitions <-
	    length(whichi(is.na(transitionTable[,1]) ||
			    which(is.na(transitionTable[,2]))))
