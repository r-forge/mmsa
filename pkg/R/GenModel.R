...
scoreSequence.GenModel <- function(x, newdata, method = c("prod", "malik",
	...
	for(i in 1:length(closestClusters)) { 
	    closestDist <- d[i,closestClusters[i]] #check if previous transition exists in model
	    previousTransition <- transition(x$model,
		    as.character(closestClusters[i]),
		    as.character(closestClusters[i]-1))
    
	    if (previousTransition > 0) previousTransition <- 1 
	    else if (previousTransition ==0) previousTransition <- -log(1/nstates(emm_test))
    ...
    if(method == "misstran") { 
	transitionTable <- transition_table(x$model, 
		newdata, method="prob", match_cluster,
		plus_one, initial_transition) 
	missingTransitions <- sum(transitionTable[,3]==0) 
	#missingTransitions <- length(whichi(is.na(transitionTable[,1]) ||
