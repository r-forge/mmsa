...
Metagenomics~\cite{sequence:Handelsman:1998} 
...
BAlibase~\cite{sequence:Smith:1981},
BLAST~\cite{sequence:Altschul:1990}, 
T-Coffee~\cite{sequence:Notredame:2000},
MAFFT~\cite{sequence:Katoh:2002}, 
MUSCLE~\cite{sequence:Edgar:2004,sequence:Edgar:2004b}, 
Kalign~\cite{sequence:Lassmann:2006} and
...
well~\cite{sequence:Thompson:1999}.
...
clustering~\cite{hahsler:Kotamarti:2010}.
Note also that the topology of the model is not
predetermined (as for HMMs~\cite{sequence:Eddy:1998}), but is learned through the associated
...
package \cite{MMSA:rEMM:2012}. In addition a limit parameter 
...
{\bf FIXME: Better way to get the actual class! No need to specify the rank in getSequences.}
...
This can be easily obtained using the \func{getClusteringDetails}.

<<>>=
getClusteringDetails(model, state=17)
@

The above command gives the sequences and the segments within them that are part of state 17. The \textit{state} parameter can be ommitted to obtain details of all the states.  To see the actual segments that are part of a state, the function \func{getClusteringSequences} can be used. It can provide both the DNA sequences as well as the NSV for a state. The commands below illustrate this. 

<<>>=
sequences <- getClusteringSequences(db, model, state=17, table="sequences")
sequences
@
<<>>=
nsv <- getClusteringSequences(db, model, state=17, table="NSV")
nsv[[1]]
@
...
In the Figure \ref{fig:Sequence_Logo}, only the first 10 sequences have been shown. The \textit{start} and \textit{end} parameters can be varied to obtain sequence logos for different segments of the sequences. Sequence logos provide a powerful way to visualize the consensus sequences at each position. Consensus sequences can also be obtained using the function \func{consensusString}.
<<>>=
