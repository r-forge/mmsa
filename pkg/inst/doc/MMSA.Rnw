...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{MMSA-abundanceplot}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{MMSA-NSVplot}
...
One of the unique advantages of MMSA is that it is able to cluster similar sequences very rapidly and accurately as compared to other methods such as multiple sequence alignment.
In this section, we will show how MMSA can be used to plot sequences and visualize similar portions or segments of sequences. Since we already have clusters and associated sequence details available as metadata information in the model, visualization is extremely fast and rapid. Thus it is a very powerful and efficient alternative to multiple sequence alignment. It can provide a visual clue about similar portions of sequences or areas of a sequence which are highly conserved across species.
...
The above command gives the sequences and the segments within them that are part of the state 17. The \textit{state} parameter can be ommitted to obtain details of all the states. If you would like to see the actual segments that are part of a state, the function \func{getClusteringSequences} comes handy. It can provide both the DNA sequences as well as the NSV for a state. The commands below illustrate this. 
...
The following command illustrates how a \textbf{Sequence Logo} plot can be obtained easily using MMSA.
...
\begin{figure}[p]
\centering
\includegraphics[width=.8\linewidth]{MMSA-EMM_4}
...
modelStatesPlot(model,17)
@

\begin{figure}[h]
\centering
\includegraphics[width=.8\linewidth]{MMSA-EMM_5}
...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{MMSA-EMM_7}
...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{MMSA-EMM_8}
...
One of the problems in bioinformatics is finding sequences or segments across sequences that are highly similar. This is especially useful in functional and phylogenetic analysis of species and sequences. As a preliminary step, MMSA includes a function \func{findLargestCommon} which searches for two or more sequences in a model that have the largest common portions. The common areas are identified by getting a count of common segments and then finding those sequences that share the maximum number of common segments.
...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{MMSA-EMM_9}
...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{MMSA-EMM_10}
