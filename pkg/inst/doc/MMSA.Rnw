...
phylogenic analysis where previously used methods do not scale 
...
{\bf FIXME: Can we compare the score for different models? What if one model has always 2 equally likely outgoing transitions? The maximal score is then only .5!}
...
{\bf FIXME: We need to make sure that the ranks come out with the first letter capitalized.}
...
<<>>=
getRank(db, rank="genus", whereRank="class", whereName="Clos")
@
Note that partial matching is performed from ``Clos'' to 
``Clostridia.'' Partial matching is available for ranks and 
...
{\bf FIXME: why do we have whereRank here but rank in nSequences()?}
