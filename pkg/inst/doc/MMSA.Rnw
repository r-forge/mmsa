...
To obtain unique sequences, getSequences can be used with rank equal to "id" and supplying the sequence's greengenes ID as the name.
This will return one sequence which can be either in a DNA Sequence format or NSV format depending on the "table" parameter.
<<>>=
s <- getSequences(db, rank="id", name="1250", table="sequences")
s
@
...
<<>>=
random_sequences <- getSequences(db, table="NSV", limit=1, random=TRUE)
random_sequences
scoreSequence(model, random_sequences[[1]],plus_one=TRUE)
@


{\bf FIXME: can we get the classification etc from the id attribute in random\_sequence?}

{\bf FIXME: maybe scoreSequence could handle more sequences at a time!}
...
In certain cases, we are interested in finding details about each state - such as which sequences and segments are part of that state. This can be easily obtained using the \func{getClusteringDetails}.

<<>>=
getClusteringDetails(model,modelState=17)
@

The above command gives the sequences and the segments within them that are part of the state 17. The \textit{modelState} parameter can be ommitted to obtain details of all the states. If you would like to see the actual segments that are part of a state, the function \func{getClusteringSequences} comes handy. It can provide both the DNA sequences as well as the NSV for a state. The commands below illustrate this. 

<<>>=
sequences <- getClusteringSequences(db, model, modelState=17, table="sequences")
nsv <- getClusteringSequences(db, model, modelState=17, table="NSV")
nsv[1:2]
@
