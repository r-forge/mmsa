...
<<>>=
true<- getRank(db, rank="Phylum", whereRank="id", whereName=names(unknown))
true
table(predicted = pred$class, true)
@
...
pred <- predict(classifier, testn)
pred

true <- getRank(db, rank="phylum", whereRank="id", whereName=test)
true

table(pred$class, true)

#confusionMatrix(pred$class, true)
@
