...
At the heart of the \pkg{QuasiAlign} package 
...
\code{all=TRUE}. The folowing code counts how many sequences we have 
for each genus.
<<>>=
table(getRank(db, rank="Genus", all=TRUE))
@

This informartion can be easily turned into a barplot showing the abundance
...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{QuasiAlign-abundanceplot}
...
names in most operations in \pkg{QuasiAlign}.
...
stores them in a directory specified by the \textit{modelDir} parameter.
...
We can see that those sequences that belong to the phylum ``Firmicutes'' have the highest score of 1.0. 
...
<<>>=
unknown <- getSequences(db, table="NSV", rank="Phylum", limit=5, random=TRUE)
classification<-classify(modelDir="models", unknown, rank="Phylum")
classification
