...
.counter <- function(x, window=100L, overlap=0L, word=3L, 
  ...
  if((is.logical(allOffsets) && allOffsets) || allOffsets==1) { 
    ...
    xc <- BiostringsTools::s2c(as.character(x)) ### sequence as a character array
    ...
    while(end <= n){
      remove_word <- BiostringsTools::c2s(xc[(start-1L):(start+word-2L)])
      add_word <- BiostringsTools::c2s(xc[(end-word+1L):end])
