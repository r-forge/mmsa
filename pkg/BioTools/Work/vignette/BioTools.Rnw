...
Database comes with R

\code{BioTools_Software_Wizard} tries to download and install the needed 
software (RDP, clustal, kalign, MAFFT, BLAST and boxshade).
...
Creating random sequences using dinocleodite trandition probabilities
<<>>=
...
applications. The following distance metrics are available in \pkg{BioTools}: 
...
\item Composition Vector (distCV): A CV is a vector with the frequencies of each k-mer in the sequency minus the expected frequency of random background nice obtained from a Markov Model (not implemented yet!). The cosine distance is used between CVs. (Qi et al, 2007).
