...
error prone. The R-based Bioconductor project~\citep{BioC} 
...
However, for multiple sequence alignment and using BLAST 
...
used for selective sequence retrieval.
...
<<sequencelogo, fig=TRUE, include=FALSE>>=
plot(al, 1, 20)
@
...
\subsection{Using the defaul RDP classifier}
...
## run BLAST (I have the NCBI database downloaded to ~/tmp/blast)
cl <- BLAST(seq, db="~/tmp/blast/refseq_rna")
cl[1:5, ]
@
