...
With this package the classifier trained with the standard training set can be used
or a custom classifier can be trained.

Current version of RDP: 2.5
}
...
and shorten the names to only the sequence ID for convenience.
<<>>=
library(rRDP)
