...
{
    if(Sys.getenv("RDP_JAR_PATH") =="") stop("Environment variable 'RDP_JAR_PATH needs to be set!'")
    if (length(list.files(classifier_dir)) > 0) stop("Classifier directory already exists! Choose a different directory or use removeRDP().")
    if (!file.exists("classifier_dir/")) dir.create("classifier")
    ...
    system(paste("java", java_args, "-cp", Sys.getenv("RDP_JAR_PATH"),"edu/msu/cme/rdp/classifier/train/ClassifierTraineeMaker ",file.path(classifier_dir,"train.txt"), file.path(classifier_dir,"train.fasta")," 1 version1 test ", classifier_dir),
	    ignore.stdout=TRUE, ignore.stderr=TRUE)
    file.copy(system.file("examples/rRNAClassifier.properties",package="RAlign"),classifier_dir)
...
removeRDP <- function(object) {
    ...
    if(!file.exists(paste(object$classifier_dir, "wordConditionalProbIndexArr.txt", sep="/"))) stop("The given RDPClassifier/directory does not look valid! Please remove the directory manually!")
